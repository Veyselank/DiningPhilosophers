...
public class Philosopher extends Thread {
	...
	{
			try {
				mutex.acquire(); // down mutex, enter critical region
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			state[i] = THINKING; //philosopher has finished eating
			test(LEFT(i)); // test if neighbour at the left can now eat
			test(RIGHT(i));// test if neighbour at the right can now eat
	...
	void take_forks(int i)
	{
			...
			state[i] = HUNGRY; // philosopher is now hungry
			test(i); // try to acquire 2 forks
			mutex.release(); //up, exit critical region
			try {
				sem[i].acquire(); // block if forks are not acquired
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
	}
	...
	{
		// GUI related stuff
		System.out.println("Philo eating");
	...
	void think()
	{
		int thinkTime = ThreadLocalRandom.current().nextInt(0,10001); // creates random variable between 0-10 secs
		state[id] = THINKING;
		System.out.println("Philo is thinking");
		try {
			sleep(thinkTime); // thinking initially
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
		table.Hungry_GUI(id); // fill the plate
	}


	void barrier() // barrier implementation
	{
		try {
			for(int i = 0; i< N-1; i++)
			{
				barriers[id].release(); // increment semaphore N-1 times
			}
			for(int i = 0; i< N; i++)
			{
				if(id != i)
					barriers[i].acquire(); // decrement all the semaphores except itself
			}
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
	...
	public void run()
	{
		...
		try
		{
				int waitTime = ThreadLocalRandom.current().nextInt(1000,10001); // creates random variable between 1-10
				sleep(waitTime); // random walk down a table
		}
		catch (InterruptedException e)
		{
			e.printStackTrace();
		}
		table.PutPlate_GUI(id); // puting table
		// Barrier
		barrier(); // wait other philosophers
		// Dining
